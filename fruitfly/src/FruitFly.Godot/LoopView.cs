...
public partial class LoopView : Node2D
{
	...
	public override void _Ready()
	{
		AddChild(new Label
		{
			Text = "Play 5 — Loop:  A ⇄ B excite each other.  Hold SPACE to kick A, then release.     (Esc = menu)",
	...
	private const float CurrentLow = 0f, CurrentHigh = 110f;   // excitatory only; ~95 peak + headroom
	private const float PlotTop = 90f, PlotBottom = 380f, BarWidth = 60f;

	private const float Ax = 80f, AbX = 240f, Bx = 400f, BaX = 560f;

	public override void _Draw()
	{
		DrawThreshold(Ax);
		DrawBar(Ax, ValueToY((float)_a.V, VLow, VHigh), _net.Fired(_a) ? Colors.White : Colors.SkyBlue);

		DrawBar(AbX, ValueToY((float)_ab.Current, CurrentLow, CurrentHigh), Colors.Orange);  // A -> B current

		DrawThreshold(Bx);
		DrawBar(Bx, ValueToY((float)_b.V, VLow, VHigh), _net.Fired(_b) ? Colors.White : Colors.SkyBlue);

...
| [0007 — The wingbeat oscillator (CPG)](plans/0007-wingbeat-cpg.md) | Isolated circuit working | Rung 1 of 0006: the motor's first self-generated rhythm. A half-centre oscillator (two mutually-inhibiting cells + the adaptation we built for memory) beats on its own from a quiet start. The circuit lives once in `FruitFly.Living.HalfCentreOscillator`; the console check and Godot **Play 7** are both thin viewers of it. Remaining: drive the fly's wings with it. |

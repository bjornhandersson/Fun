...
| [0008 — Altitude: the wingbeat holds the fly up (2.5D)](plans/0008-altitude-wingbeat-lift.md) | Not started | Give the body mass + gravity and let the wingbeat make lift, on ONE vertical axis (isolated, side-view first). A fixed beat can only fall or climb; a steady hover must EMERGE from a neural reflex (sense vertical motion → adjust beat), not a scripted setpoint. Where the beat finally earns its keep. |

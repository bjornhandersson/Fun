...
public partial class SummationView : Node2D
{
	...
	private bool _f1, _f2, _fout;           // did each neuron fire this frame? (for the flash)
	...
	public override void _Process(double delta)
	{
		...
		if (Input.IsPhysicalKeyPressed(Key.Up))   _drive1 = System.Math.Min(DriveMax, _drive1 + d);
		if (Input.IsPhysicalKeyPressed(Key.Down)) _drive1 = System.Math.Max(DriveMin, _drive1 - d);
		if (Input.IsPhysicalKeyPressed(Key.W))    _drive2 = System.Math.Min(DriveMax, _drive2 + d);
		if (Input.IsPhysicalKeyPressed(Key.S))    _drive2 = System.Math.Max(DriveMin, _drive2 - d);
	...
	// ---- Drawing ------------------------------------------------------------------
	private const float VLow = -75f, VHigh = -45f, VThreshold = -50f;
	private const float CurrentLow = 0f, CurrentHigh = 80f;   // summed current; ~2*weight peak headroom
	private const float PlotTop = 90f, PlotBottom = 380f, BarWidth = 60f;

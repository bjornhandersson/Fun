...
| [0008 — Altitude: the wingbeat holds the fly up (2.5D)](plans/0008-altitude-wingbeat-lift.md) | **Removed** (ADR 0006) | Was Play 8: `FlyingBody` with gravity, a hover reflex, and vertical chemotaxis. Deleted September 2026: the body was handed the food's height and smelled a private 1D odour field with outcome-tuned constants. Kept as a record of what not to do. |
| [0009 — Unite: a 2.5D fly + a real 3D view](plans/0009-unite-2.5d-fly-3d-view.md) | **Removed** (ADR 0006) | Was Play 9: the 2D fly and `FlyingBody` stepped side by side and drawn as one 3D creature. Deleted September 2026: two disjoint networks, forward motion misattributed to the wings. |
| [0010 — Neuralize the flight command](plans/0010-neuralize-flight-command.md) | Done (survives) | The wingbeat's "command to fly" is a real pacemaker neuron (rest above threshold, fires with zero input) whose spikes drive both wing cells through synapses. Lives on in `HalfCentreOscillator` and Play 7. |
...
| [0006](../adr/0006-remove-unfaithful-flight-plays.md) | Remove the hover and 3D-fly plays (8, 9) for fidelity; rebuild flight from the CPG as one network in one world |

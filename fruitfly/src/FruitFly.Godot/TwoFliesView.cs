...
public partial class TwoFliesView : Node2D
{
    ...
    private sealed class Fly
    {
        ...
        public double RateL,
            RateR; // smoothed motor-pop firing fractions = wheel speeds
        public double SmellLeft,
            SmellRight; // latest antenna smell readings
        public double WallLeft,
            WallRight; // latest wall-proximity readings (0 far, 1 touching)
    ...
    private const float EdgeL = 20f,
        EdgeT = 70f,
        EdgeR = 20f,
        EdgeB = 20f; // play-field margins
    ...
    public override void _Ready()
    {
        ...
        AddChild(
            ...
            {
                Text =
                    $"Play 7 — two {N:N0}-neuron flies seek honey AND avoid 5 walls, all from population firing rates. No bounce.   (Esc = menu)",
    ...
    private void StepFly(Fly fly, Vector2 size, float delta)
    {
        ...
        fly.Pos = new Vector2(
            Mathf.Clamp(fly.Pos.X, EdgeL, size.X - EdgeR),
            Mathf.Clamp(fly.Pos.Y, EdgeT, size.Y - EdgeB)
        );
        foreach (var w in _walls)
        {
            fly.Pos = PushOut(fly.Pos, w);
        }
    ...
    {
        float d = Mathf.Min(
            Mathf.Min(at.X - EdgeL, (size.X - EdgeR) - at.X),
            Mathf.Min(at.Y - EdgeT, (size.Y - EdgeB) - at.Y)
        );
        foreach (var w in _walls)
        {
            d = Mathf.Min(d, DistanceToRect(at, w));
        }
        if (d < 0f) // poked past/into a wall → treat as touching
        {
            d = 0f;
        }
    ...
    private static Vector2 PushOut(Vector2 p, Rect2 r)
    {
        ...
        float left = p.X - r.Position.X,
            right = r.End.X - p.X;
        float top = p.Y - r.Position.Y,
            bottom = r.End.Y - p.Y;
        float m = Mathf.Min(Mathf.Min(left, right), Mathf.Min(top, bottom));
        if (m == left)
        {
            return new Vector2(r.Position.X, p.Y);
        }
        if (m == right)
        {
            return new Vector2(r.End.X, p.Y);
        }
        if (m == top)
        {
            return new Vector2(p.X, r.Position.Y);
        }
    ...
    private void DrawFly(Fly fly)
    {
        ...
        DrawCircle(
            fly.Pos + AntennaDist * Heading(fly.Heading - AntennaSpread),
            3f,
            Colors.SkyBlue
        );
        DrawCircle(

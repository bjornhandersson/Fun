...
public partial class InhibitionView : Node2D
{
    ...
    public override void _Ready()
    {
        AddChild(
            ...
            {
                Text =
                    "Play 4 — Inhibition:  Input 1 (+) excites, Input 2 (−) suppresses     (Esc = menu)",
    ...
    public override void _Process(double delta)
    {
        ...
        {
            _drive1 = Mathf.Min(DriveMax, _drive1 + d);
        }
        if (Input.IsPhysicalKeyPressed(Key.Down))
        {
            _drive1 = Mathf.Max(DriveMin, _drive1 - d);
        }
        if (Input.IsPhysicalKeyPressed(Key.W))
        {
            _drive2 = Mathf.Min(DriveMax, _drive2 + d);
        }
        if (Input.IsPhysicalKeyPressed(Key.S))
        {
            _drive2 = Mathf.Max(DriveMin, _drive2 - d);
        }
    ...
    public override void _Draw()
    {
        ...
        DrawBar(
            In2X,
            ValueToY((float)_in2.V, VLow, VHigh),
            _f2 ? Colors.White : Colors.MediumPurple
        );
    ...
    private void DrawSignedBar(float x, float value, float low, float high, Color pos, Color neg)
    {
        ...
        DrawLine(
            new Vector2(x - 10, zeroY),
            new Vector2(x + BarWidth + 10, zeroY),
            Colors.Gray,
            1f
        );
        DrawRect(
            new Rect2(x, Mathf.Min(zeroY, valY), BarWidth, Mathf.Abs(zeroY - valY)),
            value >= 0 ? pos : neg
        );
    }
